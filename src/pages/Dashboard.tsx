...
const XPPanel: React.FC<{ authUser: any }> = ({ authUser }) => {
  const { stats, streak, badges, leaderboard, userRank } = useGamification(
    authUser?.id ?? null,
  );

  const loading = stats === null || streak === null || leaderboard === null;
  ...
  const xpForLevel =
    nextThreshold != null ? nextThreshold - currentThreshold : 0;
  const xpEarned =
    nextThreshold != null ? (stats?.totalXP ?? 0) - currentThreshold : 0;
  const levelProgress =
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="relative overflow-hidden bg-white/[0.06] backdrop-blur-xl rounded-3xl p-6 border border-white/10 shadow-xl">
          ...
          <div className="relative z-10 space-y-4">
            ...
            {[1, 2, 3].map((item) => (
              <div key={item} className="h-14 rounded-2xl bg-white/10" />
            ))}
          </div>
        </div>
      </div>
    ...
    <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20">
        ...
        <div className="bg-white/5 rounded-xl p-3">
          ...
          <p
            className={`text-2xl font-black ${multiplierColor(streak?.streakMultiplier ?? 1)}`}
          >
...
const Dashboard = () => {
  const achievementBadges = [
    {
      ...
      description:
        "Start your sustainability journey and earn your first eco points.",
    },
    {
      id: 2,
      name: "Ocean Saver",
      ...
      description:
        "Collect trash and help keep oceans clean by reaching 100 points.",
    },
    {
      id: 3,
      name: "Tree Guardian",
    ...
    },
    {
      ...
      description:
        "Promote responsible waste management and achieve 500 points.",
    },
    {
      id: 5,
      name: "Eco Champion",
      ...
      description:
        "Become a true environmental leader by earning 1000 eco points.",
    },
  ];
  ...
  }, [dispatch, state.notifications.length]);
  ...
  const calculateEcoScore = () => {
    ...
    const activityScore = Math.min(
      100,
      gameStats.totalTrashCollected / 10 + ecoVillage.trees / 2,
    );
    ...
    return Math.round(
      villageScore * 0.4 + activityScore * 0.3 + completionScore * 0.3,
    );
  };
  ...
  const routeFor = (text: string) => {
    ...
    if (t.includes("cleanup") || t.includes("ocean"))
      return "/ocean-cleanup-game";

    if (t.includes("water") || t.includes("tree") || t.includes("eco"))
      return "/eco-village";

    if (t.includes("learn") || t.includes("course") || t.includes("video"))
      return "/learn";
    ...
    >
      ...
      <motion.div variants={itemVariants} className="mb-8 text-center">
        <h1 className="text-4xl lg:text-6xl font-bold text-white mb-4">
          Welcome back,{" "}
          <span className="text-green-400">{authUser?.name || "Player"}!</span>
        </h1>
        <p className="text-xl text-blue-100 max-w-2xl mx-auto">
          Your environmental impact grows stronger every day. Continue your
          mission to save our planet!
        </p>
      </motion.div>
        ...
        {stats.map((stat) => {
          ...
          return (
            ...
            >
              ...
              />
              <div
                className={`w-12 h-12 rounded-xl ${stat.bgColor} flex items-center justify-center mb-4 relative z-10 border`}
              >
              ...
              <div className="relative z-10">
                ...
                {stat.change !== 0 && (
                  <div
                    className={`flex items-center text-sm mb-2 ${isPositive ? "text-green-400" : isNegative ? "text-red-400" : "text-gray-400"}`}
                  >
          ...
          <div className="mt-6 grid grid-cols-3 gap-4 text-center">
            <div className="text-white bg-white/5 rounded-xl p-3">
              ...
              <p className="text-xs text-green-400 mt-1">
                +{Math.floor(ecoVillage.trees * 0.15)} this week
              </p>
            </div>
            <div className="text-white bg-white/5 rounded-xl p-3">
        ...
        >
          <h2 className="text-2xl font-bold text-white mb-2 flex items-center">
            ...
            <span className="ml-auto text-sm text-blue-200">
              {dailyChallenges.filter((c) => c.completed).length}/
              {dailyChallenges.length} complete
            </span>
          </h2>
          {timeLeft && (
                ...
                {challenge.completed ? (
                  <div className="flex items-center justify-between">
                    <div className="flex items-center text-green-400 font-medium">
                      <TbAward className="h-4 w-4 mr-2" />
                      Completed!
                    </div>
                    <button
            ...
            return (
              ...
              >
                ...
                <p className="text-sm opacity-90 mb-2">
                  {unlocked ? "Unlocked 🎉" : `${badge.required} points needed`}
                </p>
      ...
      >
        ...
        <p className="text-lg text-white/90">
          ...
          enough oxygen for two people to breathe! Your {ecoVillage.trees} trees
          are absorbing approximately{" "}
          <span className="font-bold text-green-300">
            {(ecoVillage.trees * 48).toLocaleString()} pounds
          </span>{" "}
          of CO₂ annually! 🌳
        </p>
      </motion.div>
